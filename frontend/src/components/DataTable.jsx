...
import React, { useState, useEffect } from 'react';
import {
  Search, Filter, SortAsc, SortDesc, Trash2, RefreshCcw,
  Download, Database, Clock, Sparkles, BarChart3, Zap, Save
} from 'lucide-react';
import DSAAlgorithms from '../utils/DSAAlgorithms';
import * as XLSX from 'xlsx';

const DataTable = ({ data, onDataChange, fileName, fileOrSheetId, type }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [searchColumn, setSearchColumn] = useState('');
  const [sortColumn, setSortColumn] = useState('');
  const [sortOrder, setSortOrder] = useState('asc');
  ...
  const tableData = filteredData.length ? filteredData : editedData;
  ...
  const handleDeleteRow = (index) => {
    ...
    setFilteredData(filteredData.length ? updated : []);
    onDataChange(updated);
  ...
  if (!fileOrSheetId || !type) return;

  try {
    const token = localStorage.getItem('token');
    const url =
      type === 'sheet'
        ? `${BASE_URL}/api/users/update-sheet/${fileOrSheetId}`
        : `${BASE_URL}/api/users/update-file/${fileOrSheetId}`;
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify(
        type === 'sheet' ? { rows: editedData } : { data: editedData }
      )
    });
      ...
      <div className="flex justify-end mb-4">
        {/* {hasUnsavedChanges && ( */}
          <button 
            ...
            className="flex items-center px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg font-medium transition-all duration-200 transform hover:scale-105 shadow-lg"
          >
