...
const App = () => {
  ...
  const [dataId, setDataId] = useState(null);          // ✅ file._id or sheet._id
  const [dataType, setDataType] = useState('file');    // ✅ "file" or "sheet"
...
//   // const BASE_URL ="http://localhost:5000";
  ...
  const handleFileLoad = (data, name, _id) => {
    setCurrentData(data);
    setFileName(name);
    setSearchResults([]);
    setIsSorted(false);
    setActiveTab('table');
    setDataId(_id || null);           // ✅ Set file ID
    setDataType('file');              // ✅ Set type
  };
  ...
  const handleSheetCreate = (data, name, _id) => {
    setSheetData(data);
    setSheetName(name);
    setCurrentData(data);
    setFileName(name);
    setActiveTab('table');
    setDataId(_id || null);          // ✅ Set sheet ID
    setDataType('sheet');            // ✅ Set type
  };
          ...
          <div className="w-full px-0">
            <div className="flex justify-between items-center h-16">
              <div className="flex items-center ml-4">
                ...
                <h1 className="text-2xl font-bold bg-gradient-to-r from-slate-700 to-cyan-600 bg-clip-text text-transparent">Admin Panel</h1>
              </div>
              <div className="flex items-center space-x-4 mr-4">
                <span className="text-slate-600 font-medium">Welcome, {user.email}</span>
                <button onClick={toggleAdminMode} className="bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-4 py-2 rounded-lg flex items-center">Switch to User Mode</button>
                <button onClick={handleLogout} className="bg-gradient-to-r from-red-500 to-pink-500 text-white px-4 py-2 rounded-lg">Logout</button>
              </div>
            </div>
          </div>
        </header>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-cyan-50">
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-2xl font-bold bg-gradient-to-r from-slate-700 to-cyan-600 bg-clip-text text-transparent">DataCrafter</h1>
            </div>
            <div className="flex items-center space-x-4">
              <span className="text-slate-600 font-medium">Welcome, {user.email}</span>
              {user.email === 'sonalijatav100@gmail.com' && (
                <button onClick={toggleAdminMode} className="bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-4 py-2 rounded-lg flex items-center">
        ...
        <nav className="bg-gradient-to-r from-white/90 to-cyan-50/90 backdrop-blur-sm border-b border-slate-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex overflow-x-auto no-scrollbar space-x-4 py-2">
              {['upload', 'table', 'create', 'profile'].map(tab => {
                const Icon = tabIcons[tab];
                return (
                  <button key={tab} onClick={() => setActiveTab(tab)} className={`py-3 px-3 border-b-2 font-medium text-sm flex items-center space-x-2 ${
                    activeTab === tab
                      ? 'border-cyan-500 text-cyan-600 bg-gradient-to-t from-cyan-100/60'
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-8">
        {activeTab === 'upload' && (
          <div className="space-y-6">
            ...
            {currentData.length > 0 && (
              <div className="bg-white/90 rounded-xl shadow-xl p-6 border border-cyan-200">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={handleDownload} className="bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-6 py-2 rounded-lg flex items-center">
        ...
        {activeTab === 'table' && (
          <div className="space-y-6">
            {currentData.length > 0 ? (
              <>
                <div className="flex justify-between items-center bg-white/90 rounded-xl shadow-lg p-6 border border-cyan-200">
                  <div className="flex items-center space-x-3">
                    <div className="p-2 bg-gradient-to-r from-slate-100 to-cyan-100 rounded-lg">
                ...
                <DataTable
                  ...
                  type={dataType}
                  // fileOrSheetId={fileIdOrSheetIdFromDB}
                //  type={'sheet'} // or 'file'
                />
              </>
            ) : (
              <div className="text-center py-12 bg-white/90 rounded-xl shadow-lg border border-cyan-200">
        ...
        {activeTab === 'create' && <SheetCreator onCreateSheet={handleSheetCreate} />}
        {activeTab === 'profile' && (
          <UserProfile
            userEmail={user.email}
            onOpenSheet={({ title, rows, _id }) => {
              setCurrentData(rows);
              setFileName(title);
              setSearchResults([]);
              setIsSorted(false);
